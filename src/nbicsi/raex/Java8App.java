...
public class Java8App {
  ...
  public void run() {
    ...
    favoritesPlaylist.setSongs(
        measureExec("Sorting favorites",
            () -> favoritesPlaylist.getSongs().stream()
                .sorted((s1, s2) -> s1.getRating() >= s2.getRating() ? 1 : -1).collect(toList()))
    );
   ...
   * @param output provide a way to consume the measured value (log in out case)
  ...
  private <T> T measureExec(String details, Supplier<T> code, Consumer<String> output) {
    ...
    T result = code.get();
    try {
      // pretends to work real hard (get data from a server, DB, etc. )
      Thread.sleep(new Random().nextInt(500));
    } catch (InterruptedException ignored) {}
    output.accept(
        "- Execution of '" + details + "' took  " + (System.currentTimeMillis() - startTime)
            + "ms -");
    return result;
  }
